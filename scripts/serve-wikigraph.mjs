...
function sample(value, count) {
  ...
  return { nodes: nodes.map((n) => ({ ...n, id: n.id || n.title, url: n.url || `https://en.wikipedia.org/wiki/${encodeURIComponent(n.title).replaceAll('%20', '_')}` })), links: value.links.filter((e) => ids.has(key(e.source)) && ids.has(key(e.target))) }
}
async function sampleJsonl(count) {
  ...
  return { nodes: selected.map(({ links: _links, ...article }) => article), links }
}
createServer(async (req, res) => {
  ...
  send(res, 200, graph || { ...fallback, source: 'fallback', indexed: false })
