  ...
  for (const count of [1000, 3000]) {
    test(`${count} interleaved articles form distinct clusters in ${dimensions}D`, () => {
      ...
      const simulation = (dimensions === 2 ? forceSimulation(nodes) : forceSimulation3D(nodes, 3)).stop()
        .force('charge', (dimensions === 2 ? forceManyBody() : forceManyBody3D()).strength(-180 * repulsionScale).distanceMax(480 * spacing))
        .force('unrelated', dimensions === 2 ? unrelatedRepulsion(links, nodes, count > 2000, settings, new Set(), () => 0, repulsionScale) : null)
